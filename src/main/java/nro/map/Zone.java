...
//                            int percentDropKichHoat = Util.nextInt(0, 5000);
                                ...
                                int percentDropKichHoat = Util.nextInt(0, 200); // TỈ LỆ RỚT SET KÍCH HOẠT
...
//    public synchronized int calenderVangNgoc(long x, int type, Player p) {
    public int calenderVangNgoc(long x, int type, Player p) {
        ...
        if(type == 1) { newData = (long)p.ngoc + x; }
        if(type == 2) { newData = (long)p.ngocKhoa + x; }
